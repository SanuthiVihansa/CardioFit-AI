...
class _ECGPlotScreenState extends State<ECGPlotScreen> {
  ...
  String _errorMessage = '', _selectedLead = "Lead I"; //this has to change because Lead I may not always be there
  ...
  Widget _ecgPlotWithFocus(List<double> data, Map<String, dynamic> shapValues, double minValue, double maxValue, Color color) {
    ...
    return Padding(
        padding: EdgeInsets.only(top: _height / (_devHeight / 1), left: _width / (_devWidth / 16), right: _width / (_devWidth / 16), bottom: _height / (_devHeight / 16)),
        child: IgnorePointer(
            ...
            lineBarsData: [
              LineChartBarData(
                  ...
                  dotData: FlDotData(show: true, checkToShowDot: (spot, _) => importantTimePoints.contains(spot.x.toInt())))
            ],
        ...
        lineBarsData: [
          LineChartBarData(
              spots: ecgData.asMap().entries.map((entry) {
                int index = entry.key; double value = entry.value;
                return FlSpot(index.toDouble(), value);
              }).toList(),
              isCurved: false, colors: [Colors.blue],
              dotData: FlDotData(show: true, checkToShowDot: (spot, _) => importantTimePoints.contains(spot.x.toInt())))
        ]));
  }
}
